...
export const fetchCurrencies = createAsyncThunk<ICurrenciesResponse[], null, ThunkApiConfig>(
    ...
    async () => {
        ...
        const response = await (request({ url: `https://api.nbrb.by/exrates/rates?periodicity=0` })) as ICurrenciesResponse[];
        const data = response.filter(currency => currency.Cur_Abbreviation == "USD" || currency.Cur_Abbreviation == "EUR" || currency.Cur_Abbreviation == "RUB")
...
type TCurrenciesState = Pick<ICurrenciesResponse, "Cur_Abbreviation" | "Cur_OfficialRate">
